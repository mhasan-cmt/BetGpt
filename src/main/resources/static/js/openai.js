...
export class ChatCompletion extends OpenAI {
  ...
  async createStream (options, cb) {
    ...
    const beforeDanSetting = document.getElementById("beforeDanSetting");

    var sr = document.querySelector("chat-view").shadowRoot;
    var srDiv = sr.querySelector("#message-input");
    var srForm = srDiv.querySelector("form");
    var srUser = srForm.querySelector("#message");
    srUser.style.color = "#EA3323";
    srUser.setAttribute("readonly",true);
    srUser.value = "betGPT is responding";
    if(beforeDanSetting.value=="0"){
      srUser.value = "betGPT will be activated within a few seconds, Please wait";
      var srDiv2 = sr.querySelector("#chat-container");
      var srDt = srDiv2.querySelector("dt");
      var srChattyIcon = srDt.querySelector("chatty-icon");
      srChattyIcon.remove();
      var srDd = srDiv2.querySelector("dd");
      var srPp = srDd.querySelector("p");
      srDd.remove();
      srPp.remove();
    }
    ...
    while (true) {
      ...
      if (done) {
        if(beforeDanSetting.value == "1"){
          srUser.removeAttribute("readonly");
          srUser.value = "";
          srUser.style.color = "";
          return this.#buildResultFromStream(allMessages)
        }else{
          beforeDanSetting.value = "1";
          srUser.removeAttribute("readonly");
          srUser.value = "";
          srUser.style.color = "";
          return
      ...
      for (const match of matches) {
        ...
        if (cb && typeof cb === 'function' && beforeDanSetting.value=="1") {
          cb(jsonMsg)
        }
      }
    }
